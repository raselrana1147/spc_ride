	...
	public function settings_payment_store(Request $request)
	{
		...
		$change_content = $file;
			...
			if(!$request->has('daily_target'))
			{
				...
				if(($request->has('cash')==0 && $request->has('card')==0) && $request->has('payumoney')==0 && $request->has('paypal')==0 && $request->has('paypal_adaptive')==0 && $request->has('braintree')==0 && $request->has('paytm')==0 && $request->has('wallet')==0){
