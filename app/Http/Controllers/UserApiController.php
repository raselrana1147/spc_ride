...
{	
	...
	public function send_request(Request $request) {
		...
		$this->validate($request, [
				...
				'use_wallet' => 'numeric',
				// 'payment_mode' => 'required|in:BRAINTREE,CASH,CARD,PAYPAL,PAYPAL-ADAPTIVE,PAYUMONEY,PAYTM','WALLET',
		...
		try{
			...
			$UserRequest->geo_fencing_id = $request->geo_fencing_id;
...
//	return response()->json(['error' => $details['error_message']], 422);
...
//dd($meter);
            ...
            if(config('constants.distance','Kms')=='Kms')
                    ...
                    $kilometer = round($meter/1609.344,1); //TMi
            ...
            if($commission_percentage>0){
                ...
                $commission_price = $price + $commission_price;
            }
            ...
            $return_data['distance']=$kilometer; 
            $return_data['minute']=$minutes;    
            $return_data['time']=$time;
            $return_data['tax_price']=$this->applyNumberFormat(floatval($tax_price));    
            $return_data['base_price']=$this->applyNumberFormat(floatval($service_type->fixed));    
            $return_data['service_type']=(int)$request->service_type;   
            $return_data['service']=$fixed_price_only; 
            $return_data['geo_fencing_id']=$geo_fencing;
            $return_data['geo_service']=$geo_fencing_service_type->service_geo_fencing;   
...
dd($e);
	...
	public function chatPush(Request $request){
		...
		try{
            ...
            (new SendPushNotification)->sendPushToProvider($user_id, $message, 'default',$type);         
    ...
    public function send_otp(Request $request){
        ...
        try{
            ...
            if(count($user)){
                ...
                return response()->json(['status'=> 'failure','message'=> 'Mobile Number already registered!'], 422); 
                ...
                if($send_sms == "success"){
                    ...
                    return response()->json(['status'=> 'failure', 'message'=> $send_sms], 422);
                }
